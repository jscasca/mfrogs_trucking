...
function getCertificatesReport(){
	...
	var url = 'showMaterialCertificate.php?customer='+customer+'&project='+project+'&material='+material+'&startDate='+startDate+'&endDate='+endDate;
			...
			<div class='table' id='profitable' >
				...
				<table class="listing form" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td>
						<?
							$queryMaterial = "select * from material order by materialName asc";
							$materials = mysql_query($queryMaterial,$conexion);
							$countMaterials = mysql_num_rows($materials);
							...
							if($countMaterials > 0 ){
								if(!isset($_GET['m'])){
									...
									while($material = mysql_fetch_assoc($materials)){
										if($material['materialId'] == $_GET['c'])
										echo "<option value='".$material['materialId']."' selected='selected'>".$material['materialId']."</option>";
										else 
										echo "<option value='".$material['materialId']."'>".$material['materialName']."</option>";;
									}
								}else{
									echo "<option selected='selected'>--Material--</option>";
									while($material = mysql_fetch_assoc($materials)){
										echo "<option value='".$material['materialId']."'>".$material['materialName']."</option>";
									}
								}
						...
						<td><button id='certBtn' name='certBtn' >Certificate</button></td>
					</tr>
				</table>
			</div>
		</div>
