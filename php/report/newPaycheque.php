...
function validateForm(){
	...
	if(amount < 0){
		alert("The amount to pay can not be lower than 0.");
...
<div class="top-bar">
			...
			<div class="table">
			<form id="formValidate" name="formValidate" method="POST" action="submitPayCheque.php<?echo $additionalParams;?>" onsubmit="return validateForm();" >
				<img src="/trucking/img/bg-th-left.gif" width="5" height="4" alt="" class="left" />
				<img src="/trucking/img/bg-th-right.gif" width="4" height="4" alt="" class="right" />
				<table class="listing form" cellpadding="0" cellspacing="0">
					<tr>
						...
						$reportInfo = mysql_fetch_assoc(mysql_query("select * from report join broker using (brokerId) left join driver on (driver.driverId = report.reportType) where reportId = $reportId",$conexion));
						...
						$reportTotal = decimalPad($totalInfo['totalReported'] == null ? 0 : $totalInfo['totalReported']);
						
						$missingAmount = decimalPad($reportTotal - $paidTotal);
						?>
					<tr class="bg">
						...
						<input type="hidden" id='reportId' name='reportId' value="<?echo$reportId;?>" />
					<tr>
