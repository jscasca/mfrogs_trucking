...
WHERE
	ticketId NOT IN (select ticketId from reportticket) AND driverId=".$driver."
...
<td colspan='2'>
	<table class="subcontractor">
		...
		<tr>
			<td><?echo $projectInfo1['brokerName']?></td>
		</tr>
		<tr>
			<td>
				<table class='insurance'>
					<tr>
						<th>Driver Name:</th><td><?echo $projectInfo['driverLastName'].", ".$projectInfo['driverFirstName'];?></td>
						<th>SSN:</th><td><?echo $projectInfo['driverSSN'];?></td>
					</tr>
				</table>
			</td>
		</tr>
...
$invoicesInfo3=mysql_fetch_assoc($invoices3);
	...
	echo "<tr>";
		...
		//echo "<td align=right >".decimalPad($ticket['ticketAmount']*$ticket['itemBrokerCost']);echo"</td>";
		echo "<td align=right >".decimalPad($ticket['ticketBrokerAmount']*$ticket['itemBrokerCost']*($projectInfo['driverPercentage']/100));echo"</td>";
	echo "</tr>";
	//$subtotal+=$ticket['ticketAmount']*$ticket['itemBrokerCost'];
	//$total+=$ticket['ticketAmount']*$ticket['itemBrokerCost'];
	$total+=$ticket['ticketBrokerAmount']*$ticket['itemBrokerCost']*($projectInfo['driverPercentage']/100);
	$count++;
}
echo "<tr><td colspan='2' align=center> $count Tickets </td><td colspan='7'></td></tr>";
echo "<tr><td colspan='7'></td><th><span>Total</span></th><td align=right ><strong>".decimalPad($total);echo"</strong></td></tr>";
?>
</table>
</form>
