...
var patternLetters = new RegExp(/fakeitem/);
...
$(document).ready(function()
{
	$('#items tr td').live('dblclick',function(){
			...
			if(id!=undefined){
				...
				window.location.replace("./editProposal.php?i="+id);
				//alert(id);
				//getTicket(ticketId);
			}
...
function getItems(){
	var sprojectId = $('#sfakeprojectId').val();
	...
	$.ajax({
		type: "GET",
		url: "getEditFakeItems.php",
		data: "sfakeprojectId="+sprojectId+"&sitemNumber="+sitemNumber+"&smaterialId="+smaterialId+"&sfrom="+sfrom+"&sto="+sto,
...
function getItemNumber(fakeproject){
	$.ajax({
		type: "GET",
		url: "getFakeItemNumber.php",
		data: "fakeprojectId="+fakeproject,
		success:function(data){
			var jsonObj=jQuery.parseJSON(data);
			if(jsonObj!=null){
...
<div id="main">
	...
	<div id="middle">
		...
		?>
			...
			<div class="table">
			<form id="formValidate" name="formValidate" method="POST" action="submitEditProposal.php?i=<?echo$_GET['i']?>" onsubmit="return validateForm();" >
				<img src="/trucking/img/bg-th-left.gif" width="8" height="7" alt="" class="left" />
				<img src="/trucking/img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
				<table class="listing form" cellpadding="0" cellspacing="0">
					<tr>
					...
					<tr>
						...
						if(isset($lastVal['projectId']))
						{
							...
							echo "&nbsp;<a href='/trucking/php/view/viewShellProject.php?i=".$lastVal['projectId']."'><img src='/trucking/img/16.png' width='16' height='16' /></a>";
						}
						?> 
						</td>
					</tr>
		...
		?>
				...
				if($numitems>0)
				{
						...
						{
								echo "<tr id='fakeitem".$item['itemId']."'>";
