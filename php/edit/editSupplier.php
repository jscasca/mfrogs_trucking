...
#################
$subtitle = "Supplier";
$description = "Edit supplier. Suppliers have to be assigned to a vendor. Here you can edit the supplier general information, to edit materials and prices Values marked with <span style='color:red;'>*</span> are mandatory.";
...
$(document).ready(function()
{
	...
	$('#suppliers tr td').live('dblclick',function(){
			...
			if(id!=undefined){
				...
				window.location.replace("./editSupplier.php?i="+id);
				//alert(id);
				//getTicket(ticketId);
			}
...
<div id="main">
	...
	<div id="middle">
		...
		?>
			...
			<div class="table">
			<form id="formValidate" name="formValidate" method="POST" action="submitEditSupplier.php?a=<?echo$lastVal['addressId'];?>&i=<?echo$lastVal['supplierId'];?>" onsubmit="return validateForm();" >
				<img src="/trucking/img/bg-th-left.gif" width="8" height="7" alt="" class="left" />
				<img src="/trucking/img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
				<table class="listing form" cellpadding="0" cellspacing="0">
					<tr>
						<th class="full" colspan="2">Edit Supplier</th>
					</tr>
					<tr class="bg">
						<td class="first" width="172"><strong>Vendor:</strong><span style="color:red;">*</span></td>
						<td class="last">
						<?
						$queryTerm = "select * from vendor order by vendorName";
						...
						{	
								...
								{
									if($lastVal['vendorId']==$term['vendorId'])
					...
					<tr>
						...
						<td class="last"><input type="text" class="text" id='supplierDumptime' name='supplierDumptime' <?if(isset($lastVal['supplierDumptime']))echo"value='".$lastVal['supplierDumptime']."'";?> /></td>
					</tr>
					<tr class="bg">
						<td class="first"><strong>Additional Information:</strong></td>
						<td class="last"><textarea rows="2" cols="43" class="text" id='supplierInfo' name='supplierInfo' /> <?if(isset($lastVal['supplierInfo']))echo$lastVal['supplierInfo'];?> </textarea></td>
					</tr>
				</table>
				<table>
				<tr>
