...
if(isset($_GET['i'])){
	$lastValQuery ="select * from fuel_load where fuelLoadId=".$_GET['i'];
	$lastValReg = mysql_query($lastValQuery,$conexion);
	$lastVal = mysql_fetch_assoc($lastValReg);
}
...
$(document).ready(function()
{
	...
	$('.editable').live('click',function(){
		var id = $(this).closest('tr').find('.hiddenId').val();
		window.location = "editFuel.php?i="+id;
	});
});
...
function validateForm(){
	if($('#fuelDate').val()==""){alert("Please type a date");return false;}
...
<div id="main">
	...
	<div id="middle">
		...
		<div id="center-column">
			...
			<div class="table">
			<form id="formValidate" name="formValidate" method="POST" action="submitEditFuel.php?i=<?echo$_GET['i']?>" onsubmit="return validateForm();" >
				<img src="/trucking/img/bg-th-left.gif" width="8" height="7" alt="" class="left" />
				<img src="/trucking/img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
				<table class="listing form" cellpadding="0" cellspacing="0">
					<tr>
					...
					<tr>
						...
						<br/>
							...
							if(isset($lastVal['brokerId'])){
									...
									while($truck = mysql_fetch_assoc($trucks)){
											if($lastVal['truckId']==$truck['truckId'])
						...
						<td><input type='text' size='5px' id='fuelComment' name='fuelComment' value='<?echo$lastVal['fuelLoadCommet'];?>' /></td>
						<td><input type='text' size='4px' id='fuelStart' name='fuelStart' value='<?echo$lastVal['fuelLoadStart'];?>' /></td>
						<td><input type='text' size='4px' id='fuelFinish' name='fuelFinish' value='<?echo$lastVal['fuelLoadFinish'];?>' /></td>
						<td><label id='meterDifference'><?echo ($lastVal['fuelLoadFinish']-$lastVal['fuelLoadStart']);?></label></td>
						<td><input type='text' size='3px' id='fuelRegistered' name='fuelRegistered' value='<?echo$lastVal['fuelLoadRegistered'];?>' /></td>
						<td><input type='text' size='5px' id='fuelMileage' name='fuelMileage' value='<?echo$lastVal['fuelLoadMileage'];?>' /></td>
						<td><?if(isset($_GET['i'])){?><button id='submitFuel' >Submit</button><?}?></td>
					</tr>
					<tr class='bg'>
			...
			$numTerms = mysql_num_rows($terms);
				...
				while($term = mysql_fetch_assoc($terms))
				{
					...
					echo"<td class=fisrt'>".$term['fuelLoadDate']."<input type='hidden' value='".$term['fuelLoadId']."' class='hiddenId' /></td>";
					echo"<td >".$broker['Pid']."-".$truck['truckNumber']."</td>";
