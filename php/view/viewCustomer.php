...
$queryLast =
	...
	ON
		...
		customer.termId = term.termId and
		customerId=".$_GET['i'];
...
$(document).ready(function()
{
	$('#customers tr td').live('dblclick',function(){
			...
			if(id!=undefined){
				...
				window.location.replace("./viewCustomer.php?i="+id);
				//alert(id);
				//getTicket(ticketId);
			}
	...
	<div id="middle">
		...
		?>
			...
			<div class="top-bar">
				...
				<a onclick="return confirm('Are you sure you want to delete <?echo$lastVal['customerName'];?> and all its contacts?');"  href="deleteCustomer.php?i=<?echo$_GET['i'];?>" class="delete" ></a>
				<a href="/trucking/php/edit/editCustomer.php?i=<?echo$_GET['i'];?>" class="edit" ></a>
			</div><br />
			...
			<form id="formValidate" name="formValidate" method="POST" action="submitEditCustomer.php" onsubmit="return validateForm();" >
				...
				<table class="listing form" cellpadding="0" cellspacing="0">
					<tr>
						<th class="full" colspan="2">Customer Information</th>
					</tr>
					<tr>
						<td class="first" width="172"><strong>Name:</strong><span style="color:red;">*</span></td>
						<td class="last"> <?if(isset($lastVal['customerName']))echo$lastVal['customerName'];?></td>
					</tr>
					<tr class="bg">
						<td class="first"><strong>Phone:</strong></td>
						<td class="last"><?if(isset($lastVal['customerTel']))echo showPhoneNumber($lastVal['customerTel']);?> </td>
					</tr>
					<tr>
						<td class="first" width="172"><strong>Fax:</strong></td>
						<td class="last"> <?if(isset($lastVal['customerFax']))echo showPhoneNumber($lastVal['customerFax']);?> </td>
					</tr>
					<tr class="bg">
					...
					<tr class="bg">
						...
						<a href="
						<?
						if(isset($lastVal['customerWebsite']))
							if(strpos($lastVal['customerWebsite'],"http://")===false)
								echo"http://".$lastVal['customerWebsite'];
							else
								echo$lastVal['customerWebsite'];
						?> ">
						<? if(isset($lastVal['customerWebsite'])) echo$lastVal['customerWebsite'];?>
						</a></td>
					</tr>
					<tr>
				...
				<tr>
					<td>
						<a href="/trucking/php/new/newContact.php?i=<?echo$lastVal['customerId'];?>" >
			...
			$queryContacts = "select * from contact where customerId=".$lastVal['customerId'];
			...
			if($numTerms>0)
			{
				...
				echo "<tr>
						<th class='first' width='177'>Contacts</th>
						<th>View</th>
						<th>Edit</th>
